...
addonsettings = xbmcaddon.Addon(id='plugin.video.mc')


FILENAME = "eTV"
web='http://xbmctr.com/eTV/userfiles'
...
def main():
    web=base64.b64decode('aHR0cDovL3hibWN0ci5jb20vZVRWL3VzZXJmaWxlcy8=')
    ...
    for folder in folders:
        ...
        for videoTitle in dosya:
            ...
            tools.addDir(videoTitle,url,"","eTV.scan(url)","")   
...
def scan(Url):
    ...
    second=re.compile(u'<li><a href="(.*?)">').findall(base64.b64decode(first[0]))
    print 'second',second
    for x in second:
        url=str(Url)+str(x)
        name=x.replace(".xml","").replace("_"," ").upper()
        tools.addDir(name,url,name,"eTV.getData(name,url)")

def getData(name,url):
    bolumList=[]
    filepath=tools.check_xml_status(name,url)
    oku = open(filepath, "r")
    soup = BeautifulSOAP(oku, convertEntities=BeautifulStoneSoup.XML_ENTITIES)
