...
def Recent(url):
        ...
        try:
                ...
                for i in range (len (panel)):
                        ...
                        xbmctools1.addDir(fileName,'[COLOR gold]>>[COLOR beige][B]'+name+'[/B][/COLOR]',"ayris(url)",url,thumbnail,thumbnail)
                page=re.compile('<link rel=\'next\' href=\'(.*?)\'').findall(link)
                for url in page:
                        name=' Sonraki Sayfa' 
                        xbmctools1.addDir(fileName,'[COLOR blue][B]'+name+'[/B][/COLOR]',"Recent(url)",url,"https://koditr.org/changelog/sonrakisayfa.png")
...
##        link=xbmctools1.get_url(url)
##        soup = BeautifulSoup(link)
##        panel = soup.findAll("div", {"id": "yazi"},smartQuotesTo=None)
##        panel = panel[0].findAll("div", {"class": "tabcontents"})
##        match=re.compile('<iframe .*?src="http\:\/\/ok.ru\/videoembed\/(.*?)"').findall(str(panel))
##        for url in match:
##                name='OK.ru'
##                xbmctools1.addDir(fileName,'[COLOR pink][B]'+name+'[/B][/COLOR]',"VIDEOLINKS3(name,url)",url,'')
##        match2=re.compile('<iframe .*?src="http://videoapi.my.mail.ru/videos/embed/mail/(.*?).html"').findall(str(panel))
##        for url in match2:
##                name='Mail.ru'
##                xbmctools1.addDir(fileName,'[COLOR pink][B]'+name+'[/B][/COLOR]',"VIDEOLINKS(name,url)",url,'')
##
####                if 'youtube' in url:
####                    name='Fragman'
####                elif 'mail.ru' in url:
...
def ayris(url):
        ...
        try:       
                ...
                for code in vk_2:
                        ...
                        xbmctools1.addDir(fileName,'[COLOR lightyellow][B]'+name+'[/B][/COLOR]',"VIDEOLINKS2(name,url)",url,'')
                mailru2=re.compile('<iframe src=\'http://videoapi.my.mail.ru/videos/embed/mail/(.*?).html').findall(link)
                for code in mailru2:
                        code=''
                        name='mail RU  '
                        xbmctools1.addDir(fileName,'[COLOR blue][B]'+name+'[/B][/COLOR]',"VIDEOLINKS(name,url)",url,'')
...
def playerdenetle(name, urlList): 
        ...
        for url in urlList if not isinstance(urlList, basestring) else [urlList]: 
                ...
                if "mail.ru" in url: 
                    value.append((name,cozuculer1.MailRu_Player(url)))
                if "vk.com" in url: 
                    value.append((name,cozuculer1.magix_player(name,url))) 
