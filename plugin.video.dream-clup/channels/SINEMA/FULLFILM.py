...
def Recent(url):
        ...
        try:
                ...
                for i in range (len (panel)):
                        ...
                        name=name_fix(name)
                        xbmctools1.addDir(fileName,'[COLOR cyan][B]'+name+'[/B][/COLOR]', "ayrisdirma(name,url)",url,thumbnail,thumbnail)
                page=re.compile('<li class="active_page"><a href=".*?">.*?</a></li>\n<li><a href="(.*?)">(.*?)</a>').findall(link)
                for url,name in page:
                        xbmctools1.addDir(fileName,'[COLOR blue][B]Sonraki Sayfa >>[/B][/COLOR]'+ '[COLOR red][B]'+name+'[/B][/COLOR]', "Recent(url)",url,"https://koditr.org/changelog/sonrakisayfa.png")
...
def ayrisdirma(name,url):
        buggalo.SUBMIT_URL = denemesite
        try:
                xbmctools1.addDir(fileName,'[COLOR orange][B]>> [/B][/COLOR][COLOR beige][B]'+'Bölüm 1'+'[/B][/COLOR]',"VIDEOLINKS(name,url)",url,'')
                link=xbmctools1.get_url(url)
                soup = BeautifulSoup(link)
